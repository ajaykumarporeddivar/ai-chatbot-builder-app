...
export function useFilter<T extends Record<string, unknown>>(
  ...
  const filtered = items.filter((item) => {
    const includesSearch = fields.some((field) => item[field].toString().toLowerCase().includes(search.toLowerCase()));
    const includesStatus = status === '' || item.status === status;
    return includesSearch && includesStatus;
  });
